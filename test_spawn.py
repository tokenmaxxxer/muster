...
class GateRefusalDetection(unittest.TestCase):
    """issue #129 root causes 2/3: `gate-refusal` must fire only from the
    terminal `result` event's own `permission_denials` list, never from a
    raw-text scan over stdout lines."""

    def _run_spawn_one(self, stdout_script: str):
        import subprocess as sp
        from unittest import mock
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                with mock.patch.object(spawn, "issue_workspace",
                                        lambda cwd, issue, role: str(work)), \
                     mock.patch.object(spawn, "checkout_issue_branch",
                                       lambda cwd, issue, role: f"issue-{issue}-{role}"), \
                     mock.patch.object(spawn, "spawn_cmd",
                                       lambda *a, **k: (["python3", "-c", stdout_script], {})), \
                     mock.patch.object(spawn, "ensure_pushed",
                                       lambda *a, **k: None), \
                     mock.patch.object(spawn, "ledger_write",
                                       lambda *a, **k: None):
                    spawn._spawn_one(str(work), "qa", "맡길 일.\n",
                                     unattended=True, issue=7)
            finally:
                sys.stdout = old_stdout
                spawn.ROSTER = old_roster
    ...
    def test_no_gate_refusal_on_echoed_source_text(self):
        # issue-126 fixture: an Edit tool_result echoing spawn.py's own
        # pre-edit source, containing the literal string "denied" as data,
        # followed by a clean end_turn result with no real denials.
        script = (
            "import json\n"
            "print(json.dumps({'type': 'tool_result', 'is_error': False,\n"
            "  'content': '_DENIAL_RE = re.compile(r\\\"permission_denial|denied\\\")'}))\n"
            "print(json.dumps({'type': 'result', 'is_error': False,\n"
            "  'stop_reason': 'end_turn', 'permission_denials': []}))\n"
        )
        events = self._run_spawn_one(script)
        self.assertFalse(
            [e for e in events if e["type"] == "gate-refusal"], events)

    def test_no_gate_refusal_on_plain_end_turn_result(self):
        # core issue-46/issue-49 fixture: the result object's own JSON
        # serialization contains the substring "permission_denials" as a
        # key name even when the list is empty.
        script = (
            "import json\n"
            "print(json.dumps({'type': 'result', 'is_error': False,\n"
            "  'stop_reason': 'end_turn', 'permission_denials': []}))\n"
    ...
    def test_gate_refusal_fires_from_real_denials_in_result(self):
        script = (
            "import json\n"
            "print(json.dumps({'type': 'result', 'is_error': False,\n"
