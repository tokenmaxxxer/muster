...
|---|---|---|---|---|---|
| `product` | 무엇을 만들지 — 가치 가설 | 요구가 문제/가설 수준일 때 | hypothesis, spec (Cagan/JTBD) | [] | **keep**, scope narrowed — market-analysis split out |
| `ux-research` | 문제 → 화면·플로우 | product 스펙 확정 후 | screen/flow/wireframe spec (NN/Cooper) | [] | **keep** = current `ux-design`, renamed |
| `coding` | 승인된 범위 → 동작 코드 | 스펙/제안 승인 후 구현 | src/·test/ 코드 (Code Complete/Clean Code) | `src/**`, `test/**` | **keep**, unchanged — API/architecture decisions now arrive pre-made |
...
| `ops` | 배포 가능·계속 동작 | 머지 후 롤아웃/장애 후 | rollout checklist, postmortem (SRE Workbook) | [] | **keep**, unsplit — now also explicitly absorbs IaC/CI-CD/SLO ([[survey]] §3b) |
| `reflect` | 이슈 역사가 무엇을 가르치는가 | verify/review 종결 후 | advisory lessons (retrospective) | [] | **keep**, unchanged |
